...
Из-за того, что у таких модулей нет состояния как такового (вроде «загружен» или «не загружен»), требуется дополнить атрибут *depends* аннотацией *@NoRuntime*. для указания их в качестве зависимости атрибут depends дополняется аннотацией @NoRuntime. В этом случае зависимость будет использоваться во время сборки, но не будет определять порядок загрузки модулей относительно друг друга.
