 ...
 * mods.conf - Файл mods.conf содержит названия и опции модулей, которые будут включены в образ ОС. Также, для каждого из перечисленного в этом файле модуля возможно указать новые значения опций.
...
Список модулей для включения в конфигурацию находится в файле conf/mods.conf, который имеет следующую структуру:
...
```
В итоге, файл conf/mods.conf должен иметь следующее содержание:
